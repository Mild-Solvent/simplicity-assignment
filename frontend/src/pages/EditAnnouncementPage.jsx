...
export default function EditAnnouncementPage() {
  ...
  const previewBodyRaw   = body.trim();   // used in toast — only shown when user typed something
  const previewDate      = pubDate || 'MM/DD/YYYY HH:mm';
  const previewCats      = categories.length > 0 ? categories : [];
  const isEmpty          = !title.trim() && !body.trim() && categories.length === 0 && !pubDate;
      ...
      <div className="edit-layout">
        ...
        <div className="preview-panel">
          <div className="preview-panel-header">
            ...
            <span className="preview-panel-dot" style={{ background: '#f5a623' }} />
            <span className="preview-panel-dot" style={{ background: '#4caf50' }} />
            <span className="preview-panel-label">Notification preview</span>
          </div>
          ...
          <div className="preview-stage">
            {isEmpty ? (
              <div className="preview-empty-state">
                ...
                <p>Start filling in the form<br/>to see the notification preview</p>
              </div>
            ) : (
              /* Exact same structure + classes as the real toast */
              <div className="toast preview-toast">
                <span className="toast-icon">🔔</span>
                <span>
                  New announcement published:<br />
                  <strong>{previewTitle}</strong>
                  {previewBodyRaw && <span className="toast-body">{previewBodyRaw}</span>}
                </span>
                <button className="toast-close" tabIndex={-1} aria-hidden>×</button>
              </div>
            )}
          </div>

          <p className="preview-hint">This is exactly how the notification will look when published.</p>
        </div>
